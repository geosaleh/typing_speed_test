...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        self.label_text = Label(root, text="Welecom to typeing speed calculator", wraplength=500)
        ...
        self.label_accuracy = Label(root, text=f"Your typing accuracy is {self.speed} %")
    ...
    def stop(self):
        ...
        self.label_speed.config(text=f"Your typing accuracy is {self.speed} WPM")
        self.accuracy = round(difflib.SequenceMatcher(None,self.label_text.cget("text"),self.user_text.get("1.0",'end-1c')).ratio()*100)
        self.label_accuracy.config(text=f"Your typing accuracy is {self.accuracy} %")

    def new_text(self):
        self.label_text.config(text=self.text[random.randint(0,len(self.text)-1)])
